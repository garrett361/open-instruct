...
def main(args: FlatArguments):
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for batch_idx, short_seq_len_batch in enumerate(active_dataloader):
            if short_seq_len_batch["attention_mask"].sum() >= MAX_SEQ_LEN:
                print_rank(f"Found short batch element ({batch_idx=}): {short_seq_len_batch['attention_mask'].sum()=}\n{short_seq_len_batch=}")
                break
