...
def main(args: FlatArguments, tc: TokenizerConfig):
    ...
    with accelerator.main_process_first():
        ...
        if accelerator.is_main_process:
            ...
            print(f"========== END DEBUGGING: PRINT FIRST 3 SAMPLES AFTER CHAT TEMPLATE APPLIED by get_cached_dataset_tulu() ==========\n")
    ...
    if len(tokenizer) > embedding_size:
        ...
        # model.resize_token_embeddings(len(tokenizer), pad_to_multiple_of=8) # New embeddings are initialized as the mean of existing embeddings, generally safer for fine-tuning
        #== DQA: revise below to avoid large tensor computation that caused OOM with Bamba-9B:
        
        # TODO: Allow --mean_resizing as an input param for below script (True if not specified):
        is_bamba_model = "BAMBA" in args.model_name_or_path.upper()
        model.resize_token_embeddings(
            len(tokenizer),
            pad_to_multiple_of=8,
            **({"mean_resizing": False} if is_bamba_model else {})
        )
        if accelerator.is_main_process:
            print(f"\n========== `Bamba` in args.model_name_or_path? {is_bamba_model} ==========\n")
            print(f"========== print(torch.cuda.memory_summary()): ==========\n")
    ...
    if accelerator.is_main_process:
        ...
        for i, sample in enumerate(train_dataloader):
            ...
            if i == 2: break
        print(f"========== END DEBUGGING: PRINT SAMPLES FROM train_dataloader AFTER DataCollatorForSeq2Seq ==========\n")
    ...
    if last_checkpoint_path:
        if accelerator.is_main_process:
            accelerator.print(f"Found and Resumed from checkpoint DQA: {last_checkpoint_path}")
