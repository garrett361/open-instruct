...
#TODO: used for debugging purpose. Once confirmed, overwrite it for finetune_Bamba.py
...
def debug_apply_chat_template_and_tokenize(
    ...
    print(
        ...
        f"\n== tokenizer.special_tokens_map (len={len(tokenizer.special_tokens_map)}): {tokenizer.special_tokens_map}"
    )
...
def main(args: FlatArguments):
    ...
    if accelerator.is_main_process:
        ...
        #== DQA: PRINT OUT PARAMS
        print("\n============================= Input FlatArguments (args) =============================")
    ...
    #== DQA: Add special chat tokens if they are provided:
    ...
    #== (DQA: above just use deepspeed to get `embedding_size`, below `resize_token_embeddings` runs out of it so it's ok for both deepspeed and fsdp)
    ...
    #== DQA: Initial test for tokenizer:    
    if accelerator.is_main_process:    
        debug_apply_chat_template_and_tokenize(tokenizer)
        
    # #== DQA STOP DEBUGGING:
    # debug_STOP(accelerator)
            ...
            if accelerator.sync_gradients:
                ...
                if args.logging_steps and completed_steps % args.logging_steps == 0:
                    ...
                    metrics_to_log = {
                        ...
                        "total_tokens_including_padding": f"{total_tokens_including_padding:,}",
                        "avg_tokens_per_batch": round(avg_tokens_per_batch, 2),
                        "avg_tokens_per_batch_including_padding": round(avg_tokens_per_batch_including_padding,2),
                        "per_device_tps": round(total_tokens / accelerator.num_processes / (time.time() - start_time),2),
                        "per_device_tps_including_padding": round(total_tokens_including_padding
                        / accelerator.num_processes
                    ...
                    if args.load_balancing_loss:
                        ...
                        logger.info(
                            f"  Step: {completed_steps}, LR: {lr_scheduler.get_last_lr()[0]}, Loss: {avg_loss}, Aux Loss: {avg_aux_loss}, TPS: {total_tokens / (time.time() - start_time):.2f}"
                        )
                        metrics_to_log["aux_loss"] = avg_aux_loss
                    else:
                        logger.info(
                            f"  Step: {completed_steps}, LR: {lr_scheduler.get_last_lr()[0]}, Loss: {avg_loss}, TPS: {total_tokens / (time.time() - start_time):.2f}"
                        )
